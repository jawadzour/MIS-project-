...
public class ThesisDocumentsDaoImpl implements ThesisDocumentsDao{
    ...
    public Boolean addThesisDocuments(ThesisDocuments thesisDocuments) {
        try {
            pst = con.prepareStatement("insert into thesis_documents (thesis_document_title, thesis_group_id) values (?,?);");
            pst.setString(1, thesisDocuments.getThesisDocumentTitle());
            pst.setInt(2, thesisDocuments.getThesis().getThesisId());
            return pst.execute();
    ...
    public ThesisDocuments getThesisDocumentsById(Integer id) {
        ...
        try {
            ...
            while(rst.next()) {
                ...
                ThesisDao thesisGroupDao = new ThesisDaoImpl();
                Thesis thesisGroup = thesisGroupDao.getThesisById(rst.getInt("thesis_group_id"));
